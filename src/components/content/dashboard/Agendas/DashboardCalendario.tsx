...
import { fetchAgendamentosMes } from "@/data/server/Agendamentos";
...
export default function DashboardCalendario(props: DCalendarioProps) {
    ...
    const fetchAgendamentos = useCallback(async (month: any) => {
        try {
            processInit();
            const AgendamentosMes = await fetchAgendamentosMes(month);
    ...
    function getDayClasses(date: any) {
        const agendamentosForDate = agendamentos.filter((agendamento) => {
            ...
            {processing ? (
                ...
                <div className=" max-md:w-full grid grid-cols-7 gap-3 border-t pt-3">
                    {generateDate(today.month(), today.year()).map(
                        (
                            { date, currentMonth, today, ignore, sunday },
                            index
                        ) => {
